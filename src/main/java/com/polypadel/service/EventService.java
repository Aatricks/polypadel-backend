...
public class EventService {
    ...
    public EventResponse create(EventRequest request) {
        ...
        for (EventRequest.MatchInfo mi : request.matches()) {
            ...
            match.setTeam1(teamRepository.findById(mi.team1Id())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Équipe non trouvée")));
            match.setTeam2(teamRepository.findById(mi.team2Id())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Équipe non trouvée")));
    ...
    public void delete(Long id) {
        Event event = eventRepository.findById(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Événement non trouvé"));
        boolean hasPlayedMatches = event.getMatches().stream()
    ...
    private void validateEventRequest(EventRequest request) {
        ...
        for (EventRequest.MatchInfo mi : request.matches()) {
            if (!courts.add(mi.courtNumber())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Piste " + mi.courtNumber() + " utilisée plusieurs fois");
            }
            if (mi.team1Id().equals(mi.team2Id())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Une équipe ne peut pas jouer contre elle-même");
            }
            if (!teams.add(mi.team1Id()) || !teams.add(mi.team2Id())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Une équipe ne peut jouer qu'un match par événement");
            }
        }
    }

    private EventResponse toResponse(Event e) {
        List<MatchResponse> matches = e.getMatches().stream().map(m -> 
            new MatchResponse(m.getId(), 
                new MatchResponse.EventInfo(e.getEventDate(), e.getEventTime()),
                m.getCourtNumber(),
